...
function Home() {
    ...
    /*처음 렌더링 될 때부터 animate 되는 것 방지 <AnimatePresence initial={false}*/
    const toggleLeaving = () => setLeaving((prev) => !prev);
    const onBoxClicked = (movieId: number) => {
        navigate(`/movies/${movieId}`) //해당 url로 이동
    };
                    ...
                    <Slider>
                        <AnimatePresence initial={false} onExitComplete={toggleLeaving}>
                            <Row
                                ...
                                key={index}
                            >
                                {data?.results.slice(1).slice(offset * index, (index + 1) * offset).map((movie) => (
                                    <Box
                                        layoutId={movie.id + ""}
